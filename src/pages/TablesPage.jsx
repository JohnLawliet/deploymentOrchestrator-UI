...
function validateMetadata(payload) {
  ...
  return payload.map((descriptor, index) => {
    ...
    descriptor.columns.forEach((column, columnIndex) => {
      ...
      if (keys.has(column.key)) throw validationError(`Table "${descriptor.name}" contains duplicate column key "${column.key}".`)
      keys.add(column.key)
    })
...
function validatePage(payload) {
  ...
  for (const field of ['page', 'size', 'total']) {
    if (!Number.isInteger(payload[field]) || payload[field] < 0) {
      throw validationError(`The table page response has an invalid ${field} value.`)
    }
  }
...
function normalizeQueryResult(payload) {
  ...
  if ('items' in payload) return { ...validatePage(payload), paginated: true }
  return { items: [payload], page: 0, size: 1, total: 1, paginated: false }
}

function pagingConfig(descriptor) {
  ...
  const maximum = Number.isInteger(sizeParameter?.maximum) && sizeParameter.maximum > 0
    ? Math.min(sizeParameter.maximum, MAXIMUM_SIZE)
    : MAXIMUM_SIZE
  const requestedSize = Number.isInteger(sizeParameter?.defaultValue) && sizeParameter.defaultValue > 0
    ? sizeParameter.defaultValue
    : FALLBACK_SIZE
  return {
    page: Number.isInteger(pageParameter?.defaultValue) && pageParameter.defaultValue >= 0
...
function queryPermissionAllowed(query) {
  return query?.allowed === true
}

function errorKind(error) {
  if (error?.isValidationError || error?.status === 400 || error?.status === 422) return 'validation'
...
function formatValue(column, value) {
  ...
  if (type === 'date' || type === 'datetime') {
    ...
    return new Intl.DateTimeFormat(undefined, type === 'date'
      ? { dateStyle: 'medium' }
...
function StateNotice({ error, onRetry, title: titleOverride }) {
  const kind = errorKind(error)
  const title = titleOverride || (kind === 'forbidden'
    ? 'Access denied'
    : kind === 'validation'
  ...
  return (
    <Notice tone={kind === 'forbidden' ? 'warning' : 'error'}>
      <div>
        ...
        {onRetry && <Button variant="outline" size="sm" className="mt-3 gap-2" onClick={onRetry}><RefreshCw className="w-3.5 h-3.5" />Retry</Button>}
      </div>
    </Notice>
  )
}

function parameterIsNumeric(parameter) {
  return parameter?.name === 'page'
    || parameter?.name === 'size'
    || typeof parameter?.defaultValue === 'number'
    || typeof parameter?.maximum === 'number'
}

function parameterIsPath(parameter) {
  return String(parameter?.location || '').toLowerCase() === 'path'
}

function queryParameters(query) {
  return (Array.isArray(query?.parameters) ? query.parameters : [])
    .filter((parameter) => !isPortalIdentityParameter(parameter))
}

function initialQueryValues(query) {
  return Object.fromEntries(queryParameters(query).map((parameter) => [
    parameter.name,
    parameter.defaultValue ?? '',
  ]))
}

function QueryAccordion({ query, queryKey, activeValues, requestState, onExecute }) {
  const parameters = useMemo(() => queryParameters(query), [query])
  const [values, setValues] = useState(() => initialQueryValues(query))
  const [errors, setErrors] = useState({})

  useEffect(() => {
    setValues((current) => Object.fromEntries(parameters.map((parameter) => [
      parameter.name,
    ...
    <details className="group/query rounded-md border border-border bg-muted/20">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-3 [&::-webkit-details-marker]:hidden">
        ...
        <span className="flex min-w-0 items-center justify-end gap-2">
          <code className="hidden truncate text-right text-xs text-primary sm:block">{query.path || 'No endpoint template supplied'}</code>
      ...
      <form className="space-y-4 border-t border-border p-3" noValidate onSubmit={submit}>
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <code className="mt-1 block break-all text-xs text-primary sm:hidden">{query.path || 'No endpoint template supplied'}</code>
          </div>
        ...
        {parameters.length > 0 && (
          <div className="grid gap-3 sm:grid-cols-2">
            {parameters.map((parameter, index) => {
              ...
              return (
                <label className="block min-w-0 space-y-1.5" htmlFor={inputId} key={`${parameter.name}-${parameter.location}-${index}`}>
                  ...
                  <input
                    ...
                    max={numeric && parameter.maximum !== undefined ? parameter.maximum : undefined}
                  ...
                  />
                  {errors[parameter.name] && <span className="field-error block" role="alert">{errors[parameter.name]}</span>}
                </label>
              )
            })}
          </div>
...
function QueryCatalogue({ queries, activeQuery, requestState, onExecute }) {
  return (
    <details className="group rounded-lg border border-border bg-card">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4 text-sm font-semibold [&::-webkit-details-marker]:hidden">
        <span>Available Queries <span className="ml-1 text-xs font-normal text-muted-foreground">({queries.length})</span></span>
        <ChevronDown className="w-4 h-4 text-muted-foreground transition-transform group-open:rotate-180" />
      </summary>
      <div className="space-y-3 border-t border-border p-4">
        {queries.length === 0 && <p className="text-sm text-muted-foreground">No whitelisted operations are available for this table.</p>}
...
export default function TablesPage() {
  ...
  const [tablePage, setTablePage] = useState(null)
  const [tableError, setTableError] = useState(null)
  const [tableRefresh, setTableRefresh] = useState(0)
  const [expandedCells, setExpandedCells] = useState(() => new Set())
  const [activeQuery, setActiveQuery] = useState(null)
  const [queryRequest, setQueryRequest] = useState({ key: '', loading: false, error: null, success: '' })
    ...
    getDatabaseTables(controller.signal)
      ...
      .then((descriptors) => {
        ...
        setSelectedName((current) => {
          const selection = descriptors.find((item) => item.name === current && item.permissions.read === true)
      ...
      .catch((error) => {
        if (error?.name !== 'CanceledError') setMetadataError(error)
      })
    return () => controller.abort()
  }, [metadataRefresh])

  const descriptor = useMemo(
    () => metadata?.find((item) => item.name === selectedName) || null,
    [metadata, selectedName],
  )
  const availableQueries = useMemo(
    () => descriptor?.queries?.filter(queryPermissionAllowed) || [],
    [descriptor],
  )
  ...
  useEffect(() => {
    ...
    getDatabaseTableRows(descriptor.name, page, size, controller.signal)
      ...
      .then(setTablePage)
      .catch((error) => {
        if (error?.name !== 'CanceledError') setTableError(error)
      })
    return () => controller.abort()
  ...
  const runQuery = async (query, queryKey, values) => {
    if (queryControllerRef.current) return
    const queryLabel = query.label || query.name || queryKey
    if (query.destructive && !window.confirm(`Execute destructive query "${queryLabel}" for table "${descriptor.name}"? This action permanently deletes data and cannot be undone.`)) return
    const controller = new AbortController()
    ...
    try {
      ...
      if (queryControllerRef.current !== controller || error?.name === 'CanceledError') return
      setQueryRequest({ key: queryKey, loading: false, error, success: '' })
    } finally {
  ...
  const openDeploymentDetail = (deploymentId) => {
  ...
  const activeMaximum = Number.isFinite(Number(activeSizeParameter?.maximum))
    ? Math.min(Number(activeSizeParameter.maximum), MAXIMUM_SIZE)
    : MAXIMUM_SIZE
  ...
  const displayedSizeOptions = activeQuery
    ? [...new Set([25, 50, 100, 200, displayedSize, activeMaximum].filter((value) => value > 0 && value <= activeMaximum))].sort((a, b) => a - b)
    : sizeOptions

  const changeGridPage = (nextPage) => {
    setExpandedCells(new Set())
    if (activeQuery) runQuery(activeQuery.query, activeQuery.key, { ...activeQuery.params, page: nextPage })
    else setPage(nextPage)
  }

  const changeGridSize = (nextSize) => {
    setExpandedCells(new Set())
    if (activeQuery) runQuery(activeQuery.query, activeQuery.key, { ...activeQuery.params, size: nextSize, ...(activePageParameter ? { page: 0 } : {}) })
    else {
      setSize(nextSize)
      setPage(config.page)
    }
  ...
  return (
    <Page title="Tables" description="View operational H2 data through backend-approved metadata and endpoints.">
      ...
      {!metadata && !metadataError && <p className="flex items-center gap-2 text-sm text-muted-foreground"><Loader2 className="w-4 h-4 animate-spin" />Loading table metadata…</p>}
      {metadata?.length === 0 && <div className="empty-state"><Database className="mx-auto mb-3 h-7 w-7" />No operational tables are available.</div>}
                  ...
                  <CardContent className="space-y-4 p-4 pt-0">
                    {activeQuery && (
                      <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-primary/30 bg-primary/10 p-3">
                        <div>
                          <p className="text-sm font-medium">Query results: {activeQuery.query.label || activeQuery.query.name || activeQuery.key}</p>
                          <p className="mt-0.5 text-xs text-muted-foreground">The grid is showing the latest query response.</p>
                        </div>
                        <Button variant="outline" size="sm" disabled={queryRequest.loading} onClick={resetQuery}>Reset</Button>
                      </div>
                    )}
                    {!activeQuery && tableError && <StateNotice error={tableError} onRetry={() => setTableRefresh((current) => current + 1)} />}
                    {!displayPage && !tableError && <p className="flex items-center gap-2 py-8 text-sm text-muted-foreground"><Loader2 className="w-4 h-4 animate-spin" />Loading {descriptor.label}…</p>}
                    {displayPage && displayPage.items.length === 0 && <div className="empty-state">{activeQuery ? 'The query returned no rows.' : 'This table contains no rows for the current page.'}</div>}
                      ...
                      <div className="overflow-x-auto rounded-md border border-border">
                        <table className="w-full min-w-max border-collapse text-sm">
                          <thead className="bg-muted/55 text-left text-xs text-muted-foreground">
                            <tr>
                              {descriptor.columns.map((column) => <th className="whitespace-nowrap border-b border-border px-3 py-2.5 font-medium" key={column.key}>{column.label || column.key}</th>)}
                            </tr>
                          </thead>
                          <tbody>
                              ...
                              return (
                                <tr className="border-b border-border/70 last:border-0 hover:bg-muted/20" key={rowKey}>
                                  {descriptor.columns.map((column) => {
                                    ...
                                    const hasValue = row?.[column.key] !== null && row?.[column.key] !== undefined && value !== ''
                                    const linksToDeployment = hasValue
                                      && deploymentDetailTarget
                                      && column.key === deploymentDetailTarget.target.idField
                                    return (
                    ...
                    {displayPage && (
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          <span>{rangeStart}–{rangeEnd} of {displayPage.total}</span>
                          {(!activeQuery || (displayPage.paginated && activeSizeParameter)) && (
                            <label className="flex items-center gap-2">
                              ...
                              >
                                {displayedSizeOptions.map((option) => <option key={option} value={option}>{option}</option>)}
                              </select>
                            </label>
                          )}
                        </div>
                        {(!activeQuery || displayPage.paginated) && (
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-muted-foreground">Page {totalPages ? displayPage.page + 1 : 0} of {totalPages}</span>
                            <Button
                              variant="outline"
                              size="icon"
                              className="h-8 w-8"
                              aria-label="Previous page"
                              disabled={queryRequest.loading || displayedPage <= 0 || (activeQuery && !activePageParameter)}
                            ...
                            <Button
                              ...
                              disabled={queryRequest.loading || totalPages === 0 || displayedPage + 1 >= totalPages || (activeQuery && !activePageParameter)}
                              onClick={() => changeGridPage(displayedPage + 1)}
                            >
