...
export default function UatBuildPage() {
  ...
  useEffect(() => {
    ...
    const completedId = String(event?.resources?.operationId || event?.deploymentId || '');
  ...
  const convert = async () => {
    ...
    try {
      ...
      setConversionState('idle');
      setError(reason.message);
      if ([404, 410, 423].includes(reason.status)) releaseAndReset(reason.message);
    }
  };
      ...
      <div className="space-y-4">
        ...
        >
          <div className="space-y-4">
            ...
            {list(preflight?.missingFromJenkinsFiles).length > 0 && (
              <PathList
