...
const FRONTEND_MODES = { REUSE: 'REUSE_ASSOCIATION', DEPLOY: 'DEPLOY' };
...
export default function JarDeploymentPage() {
  ...
  const [applicationSelection, setApplicationSelection] = useState(null);
  ...
  useEffect(() => {
    ...
    getJars()
      .then((response) => {
        if (disposed) return;
        const items = Array.isArray(response) ? response : response?.jars || response?.items || response?.applications || [];
  ...
  const activity = overlayRuntimeActivity(selected || null, selected ? jarProfileActivityMap[selected.id] : null);
  ...
  const frontendAssociation = hasAuthoritativeFrontendAssociation(activity) ? activity.frontendProfile : null;
  ...
  const submit = async (event) => {
    ...
    try {
      const frontend = !includeFrontend
        ? { mode: 'NONE' }
        : frontendMode === FRONTEND_MODES.REUSE
          ? { mode: FRONTEND_MODES.REUSE, catalogueJarId: selected.id }
          : {
              mode: FRONTEND_MODES.DEPLOY,
              profileUuid: selectedFrontend.profileUuid,
      ...
      <Card className="w-full shadow-glow">
        ...
        <CardContent>
          <form onSubmit={submit} className="space-y-5">
            ...
            {profileBusy && (
              <Notice tone="warning">
                This application is currently {activity.status.toLowerCase()}. Another deployment cannot start yet.
              </Notice>
            )}
