...
vi.mock('@/components/FileBrowser', () => ({
  default: ({ rootKey, onSelectionChange }) => <button type="button" onClick={() => onSelectionChange([rootKey === 'techDrive' ? 'orders/uat.war' : 'orders/exploded'])}>Select {rootKey}</button>,
}))
vi.mock('@/components/ui/select', () => ({
  Select: ({ value, onValueChange, children, disabled }) => <select value={value} disabled={disabled} onChange={(event) => onValueChange(event.target.value)}>{children}</select>,
...
describe('UatBuildPage', () => {
  ...
  it('uses one authenticated operation stream and completes directly from its payload', async () => {
    ...
    act(() => streams[0].options.onmessage({
    ...
    act(() => streams[0].options.onmessage({
      ...
      data: JSON.stringify({ operationId: 'operation/1', status: 'COMPLETED', outputPath: 'orders/uat.war', warFileName: 'uat.war', sha256: 'abc123', message: 'Done', code: null }),
    }))
    const expectedMessage = 'Scan the war from below location and place in <clientName> SFTP.\n\ntechDrive: /tech/jonty/orders/uat.war\nHash: abc123'
