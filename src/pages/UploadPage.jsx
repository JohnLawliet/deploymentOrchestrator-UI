...
export default function UploadPage() {
  ...
  const uploadLock =
    effectiveMode === UPLOAD_MODES.REGULAR
      ? findConflictingLock?.({ section: 'UPLOAD', profile: qcDestination[0] || operation?.target?.reference, mode: 'WRITE' })
      : effectiveMode === UPLOAD_MODES.FRONTEND_HOTFIX
        ? findConflictingLock?.({
            section: 'HOTFIX',
            profile: selectedFrontend?.profileName || frontendQuery || operation?.target?.reference,
            mode: 'WRITE',
          })
        : effectiveMode === UPLOAD_MODES.WILDFLY_HOTFIX
          ? findConflictingLock?.({ section: 'HOTFIX', profile: wildflyProfileId || operation?.target?.reference, mode: 'WRITE' })
          : null;
  ...
  useEffect(() => {
    ...
    if (!current?.operationId || (!completion && (isUploadTerminal(current) || !UPLOAD_EVENT_TYPES.has(event?.eventType))))
      return;
            ...
            <Card>
              <CardContent className="space-y-4 pt-6">
                ...
                <dl className="grid grid-cols-[auto,1fr] gap-x-3 gap-y-2 text-sm">
                  ...
                  <dd className="break-all">{targetSummary || operation.target?.reference || 'Resolved by the backend'}</dd>
                </dl>
                <div>
