...
export default function DownloadsPage() {
  ...
  useEffect(() => {
    const lifecycle =
      lastSystemEvent?.eventType &&
      ['RESOURCE_ACTIVE', 'RESOURCE_FAILED', 'RESOURCE_INACTIVE'].includes(lastSystemEvent.eventType);
    ...
    >
      <Card>
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <CardDescription className="mt-1">
                {roots?.qc
                  ? `Root: ${typeof roots.qc === 'string' ? roots.qc : roots.qc.path || 'QC filesystem'}`
        ...
        <CardContent className="space-y-4">
          {error && isLockConflict(error) && (
            <Notice tone="warning">
              <strong>Resource currently locked</strong>
              <div>{error.message}</div>
              {error.users.length > 0 && <div className="mt-1">Users: {error.users.join(', ')}</div>}
          ...
          <FileBrowser
            ...
            onSelectionChange={(items, change) => {
              ...
              setSelectionTypes((current) => {
                const next = { ...current };
                const changes = change?.changes || (change ? [change] : []);
                changes.forEach((item) => {
                  if (item.selected) next[item.relative] = item.entry.type;
