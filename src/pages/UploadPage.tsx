...
export default function UploadPage() {
  const { username, frontendProfileActivityMap, wildflyProfileActivityMap, lastSystemEvent, systemStatus, findConflictingLock } =
    usePortal();
  const [mainMode, setMainMode] = useState<MainMode>('');
  const [hotfixType, setHotfixType] = useState<HotfixType>('');
  const [sources, setSources] = useState<string[]>([]);
  const [qcDestination, setQcDestination] = useState<string[]>([]);
  ...
  const [operation, setOperation] = useState<UploadResponse | null>(null);
  ...
  const effectiveMode = operation?.mode || uploadMode;
  const uploadLock =
    effectiveMode === UPLOAD_MODES.REGULAR
      ? findConflictingLock?.({ section: 'UPLOAD', profile: qcDestination[0] || '', mode: 'WRITE' })
      : effectiveMode === UPLOAD_MODES.FRONTEND_HOTFIX
        ? findConflictingLock?.({
            section: 'HOTFIX',
            profile: selectedFrontend?.profileName || frontendQuery,
            mode: 'WRITE',
          })
        : effectiveMode === UPLOAD_MODES.WILDFLY_HOTFIX
          ? findConflictingLock?.({ section: 'HOTFIX', profile: wildflyProfileId, mode: 'WRITE' })
          : null;
  ...
  const storeOperation = useCallback(
    (nextOperation: UploadResponse) => {
  ...
  const acceptOperation = useCallback(
    (nextOperation: UploadResponse, summary = '') => {
  ...
  const handleExpiredOperation = useCallback(
    (message = 'The saved upload is no longer available. It can be submitted again.') => {
  ...
  const refreshOperation = useCallback(
    async ({ quiet = false }: { quiet?: boolean } = {}): Promise<UploadResponse | null> => {
      ...
      try {
        ...
        else setError(errorMessage(reason, 'Unable to refresh the upload operation.'));
        return null;
      } finally {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (
        ...
        !Object.values(UPLOAD_MODES).includes(restored.mode as UploadMode)
      )
        throw new Error('Invalid saved upload');
      const placeholder: UploadResponse = {
        operationId: restored.operationId,
        mode: restored.mode as UploadMode,
        status: 'QUEUED',
        items: [],
        message: null,
      ...
      getUpload(restored.operationId)
        ...
        .catch((reason: unknown) => {
          if (recoverableOperationError(reason)) handleExpiredOperation();
          else setError(errorMessage(reason, 'Unable to restore the upload operation.'));
        })
  ...
  useEffect(() => {
    ...
    const completion = event.eventType === 'OPERATION_FINISHED' && ['UPLOAD', 'HOTFIX'].includes(event.resources.section);
    if (!current?.operationId || (!completion && (isUploadTerminal(current) || !UPLOAD_EVENT_TYPES.has(event.eventType)))) return;
  ...
  const confirmAbandonActive = () =>
    !running ||
    window.confirm(
      'This upload is still running. Stop displaying it and change the upload mode? The backend operation will continue.',
    );
  const changeMainMode = (value: MainMode) => {
    if (pending) return;
  ...
  const changeHotfixType = (value: HotfixType) => {
    if (pending) return;
  ...
  const requestForCurrentMode = (): UploadRequest => {
    if (uploadMode === UPLOAD_MODES.REGULAR)
      return {
        mode: uploadMode,
        sourcePaths: sources,
        target: { kind: UPLOAD_TARGET_KINDS.QC_PATH, reference: qcDestination[0] },
      };
    if (uploadMode === UPLOAD_MODES.FRONTEND_HOTFIX)
      return {
        mode: uploadMode,
        sourcePaths: sources,
        target: { kind: UPLOAD_TARGET_KINDS.FRONTEND_PROFILE, reference: frontendProfileUuid },
      };
    if (uploadMode === UPLOAD_MODES.WILDFLY_HOTFIX)
      return {
        mode: UPLOAD_MODES.WILDFLY_HOTFIX,
        sourcePaths: sources,
        target: { kind: UPLOAD_TARGET_KINDS.WILDFLY_PROFILE, reference: wildflyProfileId },
      };
    throw new Error('Select an upload mode.');
  };

  const create = async () => {
    if (!uploadMode || !sources.length || pending || uploadLock) return;
    setPending(true);
    setError('');
    setNotice('');
    try {
      ...
      const summary =
        uploadMode === UPLOAD_MODES.REGULAR
          ...
          : uploadMode === UPLOAD_MODES.FRONTEND_HOTFIX
            ? `Frontend: ${selectedFrontend?.profileName || frontendProfileUuid} · port ${selectedFrontend?.port ?? 'unknown'}`
            : `WildFly: ${selectedWildfly?.profileName || wildflyProfileId}`;
      acceptOperation(response, summary);
      setSelectedTargets({});
    } catch (reason: unknown) {
      setError(errorMessage(reason, 'Unable to create upload operation.'));
    } finally {
      setPending(false);
    }
  };
  ...
  const execute = async () => {
    ...
    const missing = (operation?.items ?? []).filter((item) => item.status === 'MISSING').length;
    ...
    try {
      ...
      setError(errorMessage(reason, 'Unable to execute upload operation.'));
    } finally {
      setPending(false);
    }
  };

  const rollback = async (item: UploadItem) => {
    if (!operationId || rollbackSource || uploadLock) return;
    if (!window.confirm(`Rollback ${item.name || item.sourcePath} at ${item.targetPath}?`)) return;
    setRollbackSource(item.sourcePath);
    setError('');
    if (operation) storeOperation(operation);
    try {
      const response = await rollbackUploadItem(operationId, item.sourcePath);
      acceptOperation(response);
      await refreshOperation({ quiet: true });
    } catch (reason: unknown) {
      setError(errorMessage(reason, 'Unable to roll back the upload item.'));
  ...
  const canCreate =
    uploadMode === UPLOAD_MODES.REGULAR
      ...
      : uploadMode === UPLOAD_MODES.FRONTEND_HOTFIX
        ? sources.length > 0 && frontendProfileUuid && selectedFrontend?.directoryExists !== false && hasFrontendIndex
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        <LockNotice lock={uploadLock} />
        {systemStatus === 'reconnecting' && operationId && !terminal && (
          <p className="text-xs text-amber-700" role="status">
            System updates are reconnecting. The upload is still running.
          </p>
        )}
        ...
        <fieldset disabled={disabled}>
          ...
          <div className="grid grid-cols-1 gap-2 lg:grid-cols-2">
            {options.map(([option, label]) => (
              <label key={option} className={`choice min-w-0 ${value === option ? 'choice-active' : ''}`}>
                <input type="radio" name={title} checked={value === option} onChange={() => onChange(option)} />
                {label}
              </label>
            ))}
          </div>
...
function RestartStatus({ operation }: { operation: UploadResponse }) {
  ...
  return (
    <div
      className={`flex items-start gap-2 rounded-md border p-3 text-sm ${failed ? 'border-red-500/40 text-red-700' : completed ? 'border-green-500/40 text-green-700' : 'border-amber-500/40 text-amber-700'}`}
      role="status"
    >
      {running ? (
      ...
      <span>
        {status === 'PENDING'
          ? 'A profile restart will be required after replacement.'
          : running
            ? 'Restarting profile'
            : completed
              ? 'Profile restart completed.'
              : `Profile restart failed${operation.message ? `: ${operation.message}` : '.'}`}
      </span>
    </div>
  );
}
...
function OperationItems({
  ...
  selectedTargets: Record<string, string>;
  setSelectedTargets: Dispatch<SetStateAction<Record<string, string>>>;
  onRollback: (item: UploadItem) => void;
...
function OperationItem({
  ...
  return (
    <div className={`rounded-md border p-2 ${tone}`} data-status={item.status}>
      <div className="flex items-center gap-2">
        <span
          className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${iconTone}`}
          title={item.status}
          data-testid="upload-item-status-icon"
        >
        ...
        {canRollback && (
          <Button
            ...
            size="sm"
            className="gap-1"
            disabled={rollingBack || selectionDisabled}
            onClick={onRollback}
          >
            {rollingBack ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RotateCcw className="h-3.5 w-3.5" />}Rollback
          </Button>
        )}
      </div>
      {warning && (
        <div className="mt-3">
          <FormLabel htmlFor={`candidate-${item.sourcePath}`}>Select target for {displayName}</FormLabel>
          <Select value={selectedTarget} onValueChange={onTargetChange} disabled={selectionDisabled}>
            <SelectTrigger id={`candidate-${item.sourcePath}`} className="mt-1">
              <SelectValue placeholder="Choose the exact target path" />
            </SelectTrigger>
            <SelectContent>
              {list(item.candidates).map((candidate) => (
      ...
      {(failure || warning) && item.message && (
        <p className={`ml-7 mt-1.5 text-xs ${failure ? 'text-red-700' : 'text-orange-700'}`}>{item.message}</p>
      )}
    </div>
  );
}
