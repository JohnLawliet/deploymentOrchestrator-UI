...
describe('FileMoveDrawer', () => {
  ...
  it('asks for overwrite confirmation then executes with overwriteConfirmed true', async () => {
    ...
    expect(onFinished).toHaveBeenCalledWith(
      expect.objectContaining({ operationId: 'move-1', completed: expect.any(Array) }),
    );
  });

  it('omits archiveFormat for same-root Move as-is by default', async () => {
  ...
  it('updates item counter from FILE OPERATION_PROGRESS events for multi plain moves', async () => {
    listFiles.mockResolvedValue([]);
