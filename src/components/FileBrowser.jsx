...
import { AlertCircle, CheckSquare, ChevronRight, File, Folder, Home, Loader2, Lock, RefreshCw, Square, Trash2 } from 'lucide-react'
...
export default function FileBrowser({ rootKey, selectableExtension, selectableType = 'any', selected = [], onSelectionChange, refreshToken = 0, disabled = false, showSelectAll = false }) {
  ...
  const load = useCallback((signal) => {
    ...
    listFiles(rootKey, path, signal).then((items) => {
  ...
  useEffect(() => { knownEntries.current.clear() }, [rootKey])
  ...
  const entryDetails = (entry) => {
    ...
    const typeAllowed = selectableType === 'directory'
      ? isDirectory
      : selectableType === 'file'
  ...
  const allVisibleSelected = selectableVisibleEntries.length > 0 && selectableVisibleEntries.every((entry) => selected.includes(joinRelative(path, entry.name)))
  const deletableSelections = selected.filter((relative) => {
    const entry = knownEntries.current.get(relative)
    return entry && !entry.locked
  })
  const navigateCrumb = (index) => { setActionError(''); setPath(index < 0 ? '.' : crumbs.slice(0, index + 1).join('/')) }
  ...
  const removeSelectedFiles = async () => {
    if (!deletableSelections.length) return
    if (!window.confirm(`Permanently delete ${deletableSelections.length} selected item(s)? Directories and their contents will be removed. This action cannot be undone.`)) return
    setDeleting(true); setActionError('')
    try {
      await deleteFiles(rootKey, deletableSelections)
      const deleted = new Set(deletableSelections)
      const changes = deletableSelections.map((relative) => ({ relative, entry: knownEntries.current.get(relative), selected: false }))
      onSelectionChange?.(selected.filter((relative) => !deleted.has(relative)), { changes })
    ...
    <div className="rounded-lg border border-border overflow-hidden">
      <div className="flex items-center justify-between gap-3 px-3 py-2 bg-muted/40 border-b border-border">
        <div className="flex items-center gap-1 flex-wrap text-xs">
          <button type="button" aria-label="Go to root directory" disabled={interactionDisabled} onClick={() => navigateCrumb(-1)} className="p-1 hover:text-primary disabled:cursor-not-allowed disabled:opacity-50"><Home className="w-3.5 h-3.5" /></button>
          {crumbs.map((crumb, index) => <span key={`${crumb}-${index}`} className="flex items-center gap-1"><ChevronRight className="w-3 h-3 text-muted-foreground" /><button type="button" disabled={interactionDisabled} onClick={() => navigateCrumb(index)} className="hover:text-primary disabled:cursor-not-allowed disabled:opacity-50">{crumb}</button></span>)}
        </div>
        <div className="flex items-center gap-1">
          {showSelectAll && supportsDelete && <Button type="button" variant="outline" size="sm" className="gap-1.5" disabled={interactionDisabled || loading || !!error || !selectableVisibleEntries.length} onClick={selectAll}>{allVisibleSelected ? <Square className="w-3.5 h-3.5" /> : <CheckSquare className="w-3.5 h-3.5" />}{allVisibleSelected ? 'Unselect all' : 'Select all'}</Button>}
          {supportsDelete && <Button type="button" variant="destructive" size="sm" className="gap-1.5" disabled={interactionDisabled || !deletableSelections.length} onClick={removeSelectedFiles}>{deleting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}{deleting ? 'Deleting…' : 'Delete'}</Button>}
          <Button type="button" aria-label="Refresh directory" variant="ghost" size="sm" disabled={interactionDisabled} onClick={() => { setActionError(''); setRefresh((value) => value + 1) }}><RefreshCw className="w-3.5 h-3.5" /></Button>
        </div>
      </div>
      {actionError && <div role="alert" className="flex items-center gap-2 border-b border-border bg-red-50 px-3 py-2 text-sm text-red-700"><AlertCircle className="h-4 w-4 shrink-0" />{actionError}</div>}
      ...
      {!loading && !error && entries.map((entry) => {
        ...
        return <div key={relative} className={`flex items-center gap-3 px-3 py-2.5 border-t border-border first:border-t-0 ${visuallyAllowed ? 'hover:bg-muted' : 'opacity-45'}`}>
          {onSelectionChange && <input type="checkbox" aria-label={`Select ${entry.name}`} checked={selected.includes(relative)} disabled={!selectionAllowed} onChange={() => toggle(relative, entry)} />}
          <button type="button" disabled={!isDirectory || interactionDisabled} onClick={() => { if (isDirectory) { setActionError(''); setPath(relative) } }} className="flex flex-1 min-w-0 items-center gap-3 text-left disabled:cursor-default">
