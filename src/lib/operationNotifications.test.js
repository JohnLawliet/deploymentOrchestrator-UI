...
const event = (section, resourceKey, resourceType = section, operationId = 'operation-1') => ({
  eventType: 'OPERATION_FINISHED',
  deploymentId: operationId,
  username: 'John Smith',
  state: 'COMPLETED',
  resources: { operationId, username: 'John Smith', section, resourceKey, resourceType, outcome: 'COMPLETED' },
});
...
describe('completion notification formatting', () => {
  it.each([
    ...
    expect(formatCompletionNotification(input, { activityMaps: maps })).toMatchObject({
      heading,
      username: 'John Smith',
      targetLabel,
