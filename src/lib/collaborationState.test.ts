...
): UserPresence => userPresence({ username, status, revision, ...extra });
const lock = (resourceKey: string, mode: LockInfo['mode'], revision: number, extra: Partial<LockInfo> = {}): LockInfo =>
  lockInfo({ resourceKey, mode, revision, owner: 'Mary Smith', expiresAt: '2099-01-01T00:00:00Z', ...extra });
const presenceEvent = (state: UserPresence['status'], resources: UserPresence) =>
  systemEvent<Extract<SystemEvent, { eventType: 'USER_PRESENCE_CHANGED' }>>({
    eventType: 'USER_PRESENCE_CHANGED',
    state,
    resources,
  });
const lockEvent = (state: 'ACQUIRED' | 'CLAIMED' | 'RELEASED' | 'EXPIRED', lockValue: LockInfo) =>
  systemEvent<Extract<SystemEvent, { eventType: 'LOCK_CHANGED' }>>({
    eventType: 'LOCK_CHANGED',
    state,
    resources: { action: state, lock: lockValue },
  });
...
describe('presence collaboration state', () => {
  it('normalizes backend usernames and replaces an early event with the authoritative snapshot', () => {
    ...
    const early = reducePresence(replacePresence([]), presenceEvent('IDLE', user('John Smith', 'IDLE', 8)));
...
describe('shared lock collaboration state', () => {
  it('reduces acquire, claim, release, expiry, and rejects older revisions', () => {
    ...
    state = reduceLocks(state, lockEvent('ACQUIRED', lock('profile:one', 'WRITE', 3)));
    state = reduceLocks(state, lockEvent('CLAIMED', lock('profile:one', 'WRITE', 4, { deploymentId: 'next' })));
    expect(state.locks['profile:one'].deploymentId).toBe('next');
    const unchanged = reduceLocks(state, lockEvent('ACQUIRED', lock('profile:one', 'WRITE', 2)));
    expect(unchanged).toBe(state);
    state = reduceLocks(state, lockEvent('RELEASED', lock('profile:one', 'WRITE', 5)));
