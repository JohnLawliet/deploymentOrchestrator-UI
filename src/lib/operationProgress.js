...
export function reconcileOperationProgress(operations, operationId, record, expectedRevision, now = Date.now()) {
  ...
  const status = record.status === null
    ...
    : DEPLOYMENT_STATUSES.has(record.status) ? record.status : previous.status ?? null
  const progressPercentage = record.progressPercentage === null
  ...
  const timestamp = typeof record.timestamp === 'string'
    ? record.timestamp
    : typeof record.lastUpdatedOn === 'string' ? record.lastUpdatedOn : new Date(now).toISOString()
  ...
  const step = { timestamp, phaseCode: phaseCode || 'UNKNOWN', message: message || '', status, progressPercentage, component: previous.component || '' }
