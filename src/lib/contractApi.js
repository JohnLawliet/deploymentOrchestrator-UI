...
const lockConflictListeners = new Set();
...
async function request(promise, fallback, notifyLockConflict = true) {
  try {
...
async function blobRequest(promise) {
  try {
    ...
    const result = normalizedError(error, 'Download failed');
...
export const validateUser = (username) =>
  request(
    client.get('/users/validate', {
      headers: techDriveHeaders(username),
    }),
    'Unable to validate this username',
  );
export const reportUserActivity = () => request(client.post('/users/activity'), 'Unable to report user activity');
export const getLocks = () => request(client.get('/locks'), 'Unable to reconcile shared locks', false);
export const onLockConflict = (listener) => {
  lockConflictListeners.add(listener);
  return () => lockConflictListeners.delete(listener);
};
...
export const getProfileDatasources = (profileId, signal) =>
  request(
    client.get(`/wildfly/profiles/${encodeURIComponent(profileId)}/datasources`, { signal }),
    'Unable to load datasources for this profile',
  );
export const getRuntimeResource = (resourceKey) =>
  request(client.get(`/resources/${encodeURIComponent(resourceKey)}`), 'Unable to load runtime activity');
export const getOperation = (id) => request(client.get(`/deployments/${encodeURIComponent(id)}`), 'Unable to load operation');
export const restartJar = (app) =>
  request(client.post(`/dashboard/jars/${encodeURIComponent(app)}/restart`, null), 'Unable to restart the application');
export const stopJar = (app) =>
  request(client.post(`/dashboard/jars/${encodeURIComponent(app)}/stop`, null), 'Unable to stop the application');
export const startProfile = (id) =>
  request(client.post(`/profiles/${encodeURIComponent(id)}/start`), 'Unable to start the profile');
...
export const getPortStatus = (port, applicationName, signal) =>
  request(
    client.get(`/system/ports/${encodeURIComponent(port)}`, {
      params: applicationName ? { applicationName } : {},
      signal,
    }),
    'Unable to inspect the port',
  );
export const getJarSnapshots = (applicationName) =>
  request(client.get('/deployments/qc/jar/snapshots', { params: { applicationName } }), 'Unable to load JAR rollback snapshots');
export const rollbackJar = (snapshotId) =>
  request(client.post('/deployments/qc/jar/rollback', { snapshotId }), 'JAR rollback was rejected');
export const preflightWar = (payload, signal) =>
  request(client.post('/deployments/qc/war/preflight', payload, { signal }), 'WAR preflight failed');
export const cancelWarPreflight = (profileId) =>
  request(
    client.delete(`/wildfly/profiles/${encodeURIComponent(profileId)}/preflight`),
...
export const getWarSnapshots = (profileId) =>
  request(client.get('/deployments/qc/war/snapshots', { params: { profileId } }), 'Unable to load rollback snapshots');
export const rollbackWar = (snapshotId, deployerName) =>
  request(client.post('/deployments/qc/war/rollback', { snapshotId, deployerName }), 'WAR rollback was rejected');
export const downloadTerminal = (deploymentId) =>
  blobRequest(client.get(`/terminals/${encodeURIComponent(deploymentId)}/download`, { responseType: 'blob' }));
export const deleteTerminal = (deploymentId) =>
  request(client.delete(`/terminals/${encodeURIComponent(deploymentId)}`), 'Unable to close the terminal');
export const subscribeProfileLogs = (profileId) =>
  request(client.put(`/profiles/${encodeURIComponent(profileId)}/log-subscriptions`), 'Unable to subscribe to profile logs');
export const unsubscribeProfileLogs = (profileId) =>
  request(
    client.delete(`/profiles/${encodeURIComponent(profileId)}/log-subscriptions`),
...
export const listFiles = (rootKey, path, signal) =>
  request(client.get('/files/list', { params: { rootKey, path }, signal }), 'Unable to load this directory');
export const downloadSingle = (rootKey, path) =>
  blobRequest(client.get('/files/download', { params: { rootKey, path }, responseType: 'blob' }));
export const downloadSelection = (rootKey, paths) =>
  blobRequest(client.post('/files/download', { rootKey, paths }, { responseType: 'blob' }));
export const deleteFiles = (rootKey, paths) =>
  request(client.delete('/files', { data: { rootKey, paths } }), 'Unable to delete the selected items');
export const preflightUatBuild = (payload, signal) =>
  request(client.post('/uat-builds/preflight', payload, { signal }), 'Unable to inspect and lock the UAT build inputs');
export const releaseUatBuildLock = (lockId) =>
  request(client.delete(`/uat-builds/locks/${encodeURIComponent(lockId)}`), 'Unable to release the UAT build lock');
export const convertUatBuild = (payload) =>
  request(client.post('/uat-builds/convert', payload), 'Unable to start UAT build conversion');
export const createUpload = (payload) => request(client.post('/uploads', payload), 'The upload was rejected');
export const getUpload = (operationId) =>
  request(client.get(`/uploads/${encodeURIComponent(operationId)}`), 'Unable to load the upload operation');
export const executeUpload = (operationId, selectedTargets) =>
  request(
    client.post(`/uploads/${encodeURIComponent(operationId)}/execute`, { selectedTargets }),
    'The hotfix could not be started',
  );
export const rollbackUploadItem = (operationId, sourcePath) =>
  request(
    client.post(`/uploads/${encodeURIComponent(operationId)}/rollback`, { sourcePath }),
    'The item rollback could not be started',
  );
export const getDatabaseTables = (signal) =>
  request(client.get('/database/tables', { signal }), 'Unable to load database table metadata');
export const getDatabaseTableRows = (table, page, size, signal) =>
  request(
    client.get(`/database/tables/${encodeURIComponent(table)}`, { params: { page, size }, signal }),
...
export const isLockConflict = (error) => error?.status === 423;
